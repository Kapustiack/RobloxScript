...
-- Reused across every refresh instead of building a fresh OverlapParams (and
-- relying on Lua-side "seen" bookkeeping) each call. Filtering the character
-- out at the engine level is both cheaper and safer than force-adding
-- unfiltered GetPartsInPart results.
local overlapParams = OverlapParams.new()
overlapParams.FilterType = Enum.RaycastFilterType.Exclude
overlapParams.MaxParts = TRACK_LIMIT

getgenv().noclipPreset = getgenv().noclipPreset or "both"
local NOCLIP_PRESET_LABELS = {character = "Character Only", vehicle = "Vehicle Only", both = "Both"}
local NOCLIP_PRESET_ORDER = {"character", "vehicle", "both"}
...
local function refreshTrackedParts(char, hrp, hum)
    local preset = getgenv().noclipPreset or "both"
    local includeChar    = preset == "character" or preset == "both"
    local includeVehicle = preset == "vehicle"   or preset == "both"
    ...
    if includeVehicle and seat then
        add(seat, true)
    ...
    if hrp then
        pcall(function()
            ...
            for _, part in ipairs(workspace:GetPartsInPart(hrp, overlapParams)) do
                add(part, true)
                if #tracked >= TRACK_LIMIT then break end
            end
        end)
...
-- Records the last CFrame where the character had solid ground within a
-- short distance below them, so disableNoclip() can recover if the player
-- toggles off while stuck inside geometry or over the void.
local function updateLastSafePosition(hrp, char)
    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Exclude
    params.FilterDescendantsInstances = {char}
    local result = workspace:Raycast(hrp.Position, Vector3.new(0, -12, 0), params)
    if result then
        getgenv().noclipLastSafePos = hrp.CFrame
    end
end
        ...
        if not part or not part.Parent then
            ...
            if shouldAdjustDensity and not isCharacterPart then
                pcall(function()
                    ...
                    local cur = part.CustomPhysicalProperties
                    -- Skip the reallocation/write when density is already
                    -- correct. This loop runs on every Stepped tick for up to
                    -- TRACK_LIMIT parts, so writing unconditionally caused
                    -- needless per-frame allocations and property churn.
                    if not cur or math.abs(cur.Density - density) > 1e-4 then
...
local function enableNoclip()
    ...
    getgenv().noclipTrackerConnection = RunService.Heartbeat:Connect(function()
        ...
        updateLastSafePosition(hrp, char)
    end)
...
local function disableNoclip()
    ...
    -- Safety net: if we end up with no solid ground nearby (noclipped deep
    -- into terrain or off the edge of the map), snap back to the last
    -- position where we had ground under us instead of falling into the
    -- void or staying trapped in geometry.
    task.spawn(function()
        task.wait(0.1)
        local char = LocalPlayer.Character
        ...
        if not result and getgenv().noclipLastSafePos then
            hrp.CFrame = getgenv().noclipLastSafePos
        end
    end)
end
...
local function refreshNoclipPresetButton()
    local btn = getgenv().NoclipPresetBtn
    if not btn then return end
    btn.Text = "Preset: " .. (NOCLIP_PRESET_LABELS[getgenv().noclipPreset] or "Both")
end

if getgenv().NoclipPresetBtn then
    getgenv().NoclipPresetBtn.MouseButton1Click:Connect(function()
        local cur = getgenv().noclipPreset or "both"
        local idx = 1
        for i, v in ipairs(NOCLIP_PRESET_ORDER) do if v == cur then idx = i; break end end
        getgenv().noclipPreset = NOCLIP_PRESET_ORDER[(idx % #NOCLIP_PRESET_ORDER) + 1]
        refreshNoclipPresetButton()
        -- Force an immediate re-scan so the new preset takes effect right away.
        getgenv().noclipTrackedParts = {}
        getgenv().noclipLastRefresh = 0
    end)
    refreshNoclipPresetButton()
...
getgenv().refreshNoclipPresetButton = refreshNoclipPresetButton
