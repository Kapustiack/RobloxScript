...
function Hooks:InstallMainHook(config)
    ...
    local function hookedIndex(self, key)
        if not checkcaller() and config.GetScriptEnabled() then
            -- Only spoof the LOCAL player's own Humanoid. Without this check the
            -- fake WalkSpeed/JumpPower leaked into every read of every player's
            -- Humanoid on the server (e.g. any local script inspecting another
            -- player's speed would get our fake value instead of theirs).
...
-- Expose to the shared global environment (the loader discards module returns).
getgenv().Hooks = Hooks
